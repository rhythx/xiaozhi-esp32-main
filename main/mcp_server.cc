...
void McpServer::DoToolCall(int id, const std::string& tool_name, const cJSON* tool_arguments, int stack_size) {
    ...
    // 使用 BackgroundTask 代替 std::thread
    Application::GetInstance().GetBackgroundTask()->Schedule([this, id, tool_iter, arguments = std::move(arguments)]() {
        try {
            ReplyResult(id, (*tool_iter)->Call(arguments));
        } catch (const std::exception& e) {
            ESP_LOGE(TAG, "tools/call exception in background task: %s", e.what());
            ReplyError(id, e.what());
        }
    });

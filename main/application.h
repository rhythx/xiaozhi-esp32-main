...
public:
    ...
    BackgroundTask* GetBackgroundTask() { return background_task_; }
    MQTTData* GetMqttData() { return mqtt_data_; }// 获取MQTT数据对象
private:
    Application();
    ~Application();
    ...
    // WiFi连接事件回调
    static void OnWiFiConnected(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data);

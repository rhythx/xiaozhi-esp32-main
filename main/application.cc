...
Application::Application() {
    // 注册WiFi事件处理器
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, reinterpret_cast<esp_event_handler_t>(&Application::OnWiFiConnected), this));

    // **修改**: 创建app_car_实例
    app_car_ = new AppCar();
    mqtt_data_ = nullptr; // **修改**: 确保mqtt_data_初始化为nullptr
...
Application::~Application() {
    ...
    // 注销WiFi事件处理器
    esp_event_handler_unregister(IP_EVENT, IP_EVENT_STA_GOT_IP, reinterpret_cast<esp_event_handler_t>(&Application::OnWiFiConnected));
...
//WiFi连接成功后的回调函数，这是启动网络服务的关键点
void Application::OnWiFiConnected(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    Application* app = static_cast<Application*>(arg);
    ESP_LOGI(TAG, "WiFi connected, starting MQTT and Car services.");
    
    // 创建MQTTData实例并启动其服务 (TCP Server & MQTT Client)
    if (app->mqtt_data_ == nullptr) {
        app->mqtt_data_ = new MQTTData();
        app->mqtt_data_->StartServer();

        // MQTTData创建后，立即用其指针初始化AppCar模块（包括UART）
        if (app->app_car_) { 
            app->app_car_->Init(app->mqtt_data_);
        }
    }
}
 ...
 * @brief 在设备上显示警报消息。
 * 
 * 此函数记录警报详细信息，设置显示状态和表情，并显示聊天消息。
 * 如果提供了声音，它会重置解码器并播放声音。
 * 
 * @param status 要显示的状态字符串。
 * @param message 要显示的消息字符串。
 * @param emotion 要显示的情绪字符串。
 * @param sound 要播放的声音文件（可选）。如果为空，则不播放声音。
 */
...
void Application::Start() {
    ESP_LOGI(TAG, "Application starting, waiting for WiFi connection...");
    auto& board = Board::GetInstance();
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                if (cJSON_IsString(text)) {
                    ...
                    // 检查LLM的回复是否是结束对话的指令
                    if (strstr(text->valuestring, "好的，等会再聊") != NULL) {
                        Schedule([this]() {
                            // 停止当前的音频活动并返回空le闲状态
            ...
            if (cJSON_IsString(text)) {
                ...
                // 捕获到STT文本后，调用app_car的ProcessSTTText函数处理
                if(app_car_) {
                    app_car_->ProcessSTTText(text->valuestring);
                }
            }
        }
        // 处理LLM消息 
...
// ... (remaining functions in application.cc can be kept as they are) ...
